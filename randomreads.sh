...
Read names generated by this program are used by MakeRocCurve (samtoroc.sh) and GradeSamFile (gradesam.sh).
...
With addcigar=t, embeds a truth CIGAR string in the read header for alignment validation.
...
addcigar=f      Embed a truth CIGAR string (=/X/I/D) in the read header.
                Used by gradesam.sh (with fastq= flag) to grade alignment
                accuracy at the indel level, not just position.
...
launch() {
	CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP synth.RandomReads3 build=1 $@"
	echo "$CMD" >&2
	java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" synth.RandomReads3 build=1 "$@"
}
