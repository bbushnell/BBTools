...
Description:  Accurate-but-slow read aligner; BBMapAcc variant of BBMap (exhaustive slowWalk2, skips fast heuristics for higher mapping accuracy).
Please read bbmap/docs/guides/BBMapGuide.txt for more information.

Usage:                      bbmapacc.sh ref=<fasta> in=<reads> out=<sam>
Index only:                 bbmapacc.sh ref=<fasta>
Map to existing index:      bbmapacc.sh in=<reads> out=<sam>
Map without writing index:  bbmapacc.sh ref=<fasta> in=<reads> out=<sam> nodisk
...
launch() {
	CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP align2.BBMapAcc build=1 overwrite=true fastareadlen=500 $@"
	echo "$CMD" >&2
	java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" align2.BBMapAcc build=1 overwrite=true fastareadlen=500 "$@"
}
