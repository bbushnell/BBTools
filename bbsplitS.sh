...
Uses BBMapS with Streamer/Writer input and split output.
Supports mapmode=normal; use bbsplit.sh for accurate/PacBio mapping modes.

Usage: bbsplitS.sh ref=x.fa,y.fa in=reads.fq basename=o%.fq
that is equivalent to
bbsplitS.sh build=1 in=reads.fq ref_x=x.fa ref_y=y.fa out_x=ox.fq out_y=oy.fq

To index:     bbsplitS.sh build=<1> ref_x=<reference fasta> ref_y=<another reference fasta>
To map:       bbsplitS.sh build=<1> in=<reads> out_x=<output file> out_y=<another output file>
...
Exceptions include the 'nodisk' flag, which BBSplitS does not support.
BBSplitS is recommended for fastq and fasta output, not for sam/bam output.
When the reference sequences are shorter than read length, use Seal instead of BBSplitS.
...
launch() {
	CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP align2.BBMapSplitterS ow=t fastareadlen=500 minhits=1 minratio=0.56 maxindel=20 qtrim=rl untrim=t trimq=6 $@"
	echo "$CMD" >&2
	java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" align2.BBMapSplitterS ow=t fastareadlen=500 minhits=1 minratio=0.56 maxindel=20 qtrim=rl untrim=t trimq=6 "$@"
}
