...
maxnrate=1          If below 1, reads with a greater fraction of Ns than
                    this will be discarded.  Unlike maxns this scales with
                    read length, so maxnrate=0.75 excludes junk sequences
                    of any length.  May be used with maxns.
...
launch() {
	...
	echo "$CMD" >&2
	java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" jgi.BBDuk "$@"
}
