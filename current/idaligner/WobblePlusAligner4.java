...
public class WobblePlusAligner4 implements IDAligner{
	...
	public static final float alignStatic(byte[] query0, byte[] ref0, int[] posVector) {
		...
		for(int i=1; i<=qLen; i++){
			...
			//from two rows ago (double buffering); a re-widened band exposes them. This
			//variant is SIMD-only, and the vector routine receives prev as-is, so this
			//pre-dispatch clear is the entire fix.
