...
public class DriftingPlusAligner3 implements IDAligner{
	...
	public static final float alignStatic(byte[] query0, byte[] ref0, int[] posVector) {
		...
		// drift can be negative here, so both edges can re-expose stale cells; clearing
		// before the SIMD/scalar dispatch covers both paths.
...
//			prevRowScore=maxScore; //Not needed
			...
			int posFromSimd=0;
			if(Shared.SIMD) {
				posFromSimd=simd.SIMDAlign.alignBandVectorAndReturnMaxPos2(q, ref, bandStart, bandEnd, prev, curr);
			}else {
