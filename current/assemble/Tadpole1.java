...
//			if(verbose  /*|| true*/){outstream.println("Thread "+id+" checking owner: "+findOwner(bases, bases.length, id));}
	...
	private void invalidateCrossKInternal(final byte[] bases, final int invalidOwner){
		if(bases.length<=k+1){return;}
		long kmer=0, rkmer=0;
		int len=0;
		for(int i=0; i<bases.length; i++){
			...
			final int start=i-k+1;
			if(len>=k && start>0 && start<bases.length-k){tables.claim(kmer, rkmer, invalidOwner);}
		}
	}

	/** Claims one endpoint, invalidating it if ineligible or already seen anywhere. */
	private void claimCrossKEnd(final long kmer, final boolean eligible, final int owner, final int invalidOwner){
		final long rkmer=rcomp(kmer);
		final int old=tables.findOwner(kmer, rkmer);
		tables.claim(kmer, rkmer, eligible && old<0 ? owner : invalidOwner);
	}
...
//			assert(tracker.detectedReassemble>0) : counts;
		...
		//Item 3b: substitute runs strictly after reassemble, only as cleanup for errors reassemble
...
//				}
	...
	private int getCountWithSubstitution(final byte[] bases, final int windowStart, final int subPos, final int alternateBase){
		long kmer=0;
		for(int i=windowStart; i<windowStart+k; i++){
			final long x=(i==subPos ? alternateBase : AminoAcid.baseToNumber[bases[i]]);
