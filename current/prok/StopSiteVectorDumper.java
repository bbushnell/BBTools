...
/**
 * Generates stop-site training vectors for a dedicated StopNet.
 * For each stop codon that anchors at least one ORF candidate, outputs one-hot bases
 * around the stop site with label=1 if any NCBI CDS ends there, 0 otherwise.
 * One vector per unique stop position (not per ORF candidate).
 * @author Brian Bushnell, Chloe
 */
public class StopSiteVectorDumper extends ProkObject {

	public static void main(String[] args){
		Timer t=new Timer();
		StopSiteVectorDumper x=new StopSiteVectorDumper(args);
		x.process(t);
		Shared.closeStream(x.outstream);
	}

	public StopSiteVectorDumper(String[] args){
		{
	...
	void process(Timer t){
		...
		long totalStops=0, totalPositive=0, totalNegative=0;
		...
		for(int fnum=0; fnum<fnaList.size(); fnum++){
			...
			HashSet<String> ncbiStops=loadNcbiStops(gff);
			...
			for(Read r : reads){
				...
				for(Orf orf : orfs){
					...
					if(seenStops.contains(stopKey)){continue;}
					seenStops.add(stopKey);

					boolean match=ncbiStops.contains(stopKey);

					ByteBuilder bb=new ByteBuilder();
					appendStopVector(bb, orf, bases, gc, match); //gc is now per-genome
					bsw.print(bb);
			...
			if((fnum+1)%100==0){
				outstream.println("Processed "+(fnum+1)+"/"+fnaList.size()+" files, "+totalStops+" stops ("+totalPositive+" pos, "+totalNegative+" neg)");
			}
		}

		bsw.poisonAndWait();
		t.stop();
		outstream.println("Total stop sites: "+totalStops+" ("+totalPositive+" positive, "+totalNegative+" negative)");
		outstream.println("Positive rate: "+String.format("%.4f", totalPositive/(double)Tools.max(1, totalStops)));
		outstream.println(Tools.timeReadsBasesProcessed(t, fnaList.size(), totalStops, 8));
	}

	/** Load NCBI CDS stop positions as "seqid:stop1based:strand" keys. */
	private HashSet<String> loadNcbiStops(String gffFile){
		...
		for(GffLine gl : cdsList){
			...
			int idx=seqid.indexOf(' ');
			if(idx>=0){seqid=seqid.substring(0, idx);}
			set.add(seqid+":"+gl.stop+":"+gl.strand);
		}
		return set;
	}

	private void appendStopVector(ByteBuilder bb, Orf orf, byte[] bases, float gc, boolean match){
	...
	private int upstream=9;
	private int downstream=21;
