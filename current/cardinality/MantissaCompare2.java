...
/**
 * Multi-threaded single-bucket 16-bit simulation comparing sub-NLZ
 * extraction methods for cardinality estimation.
 * Extends MantissaCompare with threading, geometric/harmonic CF averaging,
 * sample-mode control, and additional modes (HistMant, TwinTail,
 * MasterSlave, Super1Bit, AvgNlz, Median3, MinPlus1, CTLL).
 *
 * State 0 = Established/Small Gap. State Cap = Outlier/Big Gap.
 *
 * Usage: java cardinality.MantissaCompare2 [mode=mantissa] [bits=2]
 *        [inner=32768] [outer=131072] [maxtier=11] [threads=1]
 *        [sample=all|entry|both] [hbits=N] [mbits=N]
 *
...
public class MantissaCompare2 {
	...
	/** Parse arguments and run simulation across multiple threads. */
				...
				for(int trial=trialStart; trial<trialEnd; trial++){
					...
					for(int card=1; card<=fInner; card++){
						...
						if(fMode==MODE_SUPER1BIT){
							...
							// Tier ratio = (sqrt(2))^3 = 2*sqrt(2) approx 2.828.
