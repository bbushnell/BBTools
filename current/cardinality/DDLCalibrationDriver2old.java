 ...
 * @date March 2026
 */
public class DDLCalibrationDriver2old {
	...
	public static void main(String[] args){
		...
		final CalibrationThread2old[] calThreads=new CalibrationThread2old[numThreads];
		for(int t=0; t<numThreads; t++){
			final int threadDDLs=numDDLs/numThreads+(t<numDDLs%numThreads ? 1 : 0);
			calThreads[t]=new CalibrationThread2old(
				masterSeed+t, threadDDLs, thresholds, buckets, k, maxTrue, loglogtype, out4, dupFactor);
		}
		for(CalibrationThread2old ct : calThreads){ct.start();}

		// Wait for all threads
		for(CalibrationThread2old ct : calThreads){
	 ...
	 */
	static final class CalibrationThread2old extends Thread {

		CalibrationThread2old(long seed, int numDDLs, long[] thresholds,
