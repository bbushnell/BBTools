 ...
 * Feeds 50M random hashes through one bucket, tracking tier advances
 * and history transitions to verify steady-state P(hist=0) against
...
public class TestSingleBucket {
	...
	public static void main(String[] args){
		...
		for(int i=0; i<50_000_000; i++){
			...
			final int elemTier=(2*rawNlz+mant)/3;

			if(!occupied){
				storedTier=elemTier; hist=0; occupied=true;
			}else{
				final int delta=elemTier-storedTier;
				if(delta>0){
					// Rule 1 & 2: advance
					hist=(delta==1) ? 1 : 0;
		...
		final long totalAdv=advSet+advClr;
