...
/**
 * Arithmetic-encoded variable-history DLL with 11 buckets per 64-bit word.
 * RADIX=56, unsigned 64-bit. Single long[] array, no extra byte[].
 * T0=1 state, T1=2 states, T2+=4 states. HIST_STATES = 4*HTL - 1.
 *
 * @author Noire
 * @date May 2026
 */
public final class ArithmeticVariableDynamicLogLog64 extends CardinalityTracker {

	/*--------------------------------------------------------------*/
	/*----------------        Initialization        ----------------*/
	/*--------------------------------------------------------------*/

	ArithmeticVariableDynamicLogLog64(){this(2048, 31, -1, 0);}

	ArithmeticVariableDynamicLogLog64(parse.Parser p){
		this(p.loglogbuckets, p.loglogk, p.loglogseed, p.loglogMinprob);
	}

	ArithmeticVariableDynamicLogLog64(int buckets_, int k_, long seed, float minProb_){
	...
	public final void add(CardinalityTracker log){
		...
		add((ArithmeticVariableDynamicLogLog64)log);
	}

	public void add(ArithmeticVariableDynamicLogLog64 log){
		added+=log.added;
		lastCardinality=-1;
