...
/**
 * Original LogLog cardinality estimator (pre-CardinalityTracker hierarchy).
 * Uses tabulated hash functions and a bucket array to estimate distinct k-mers.
 * Supports both atomic (AtomicIntegerArray) and non-atomic (int[]) modes.
 *
...
public class LogLog_old {
	...
	/** Default constructor: 1999 buckets, 8 bits, k=31. */
	public LogLog_old(){
		this(1999, 8, 31, -1, 0);
	}
...
//	}
	...
	public final long cardinality(double mult){
		...
		final double mean=sum/(double)buckets;
		final long cardinality=(long)((((Math.pow(2, mean)-1)*buckets*SKIPMOD))/1.258275);
	...
	public final long cardinalityH(){
		double sum=0;
		for(int i=0; i<maxArrayA.length(); i++){
			final int x=Tools.max(1, maxArrayA.get(i));
			sum+=1.0/x;
		}
...
//	}
	...
	/** Hashes a value using the tabulated hash function. */
	...
	/** Hashes all k-mers from a Read and its mate (if present). */
	...
	/** Hashes small k-mers (k<32) from bases with optional quality filtering. */
	...
	/** Hashes big k-mers (k>=32) from bases with optional quality filtering. */
	...
	/** Merges another LogLog_old into this one via per-bucket max. */
	...
	/** Hashes a single number and stores the leading-zero count in the appropriate bucket. */
...
//		final int bucket=(int)((number&Integer.MAX_VALUE)%buckets);
		...
		if(atomic){
			...
			while(leading>x){
				final boolean b=maxArrayA.compareAndSet(bucket, x, leading);
				if(b){x=leading;}
	...
	/** Generates a tabulated hash function with the given number of steps and bits. */
	private static long[][] makeCodes(int length, int bits, long seed){
		final Random randy=Shared.threadLocalRandom(seed);
		final int modes=1<<bits;
		final long[][] r=new long[length][modes];
...
//					throw new RuntimeException("Unknown parameter "+args[i]);
		...
		void process(){
			...
			final LogLog_old log=new LogLog_old(buckets, bits, k, seed, minProb);
			...
			for(int ffnum=0; ffnum<ffin1.length; ffnum++){
				...
				for(LogLogThread llt : threads){llt.start();}
			...
			if(out!=null){ReadWrite.writeString(cardinality+"\n", out);}
