...
/**
 * HalfCappedDynamicLogLog4: CDLL4 variant with alternating overflow handling.
 * <p>
 * Even buckets (bucketNum&1==0) cap overflow at tier 7 (standard CDLL4 behavior).
 * Odd buckets (bucketNum&1==1) ignore overflow (skip update if tier exceeds 7).
 * Capping inflates estimates under duplicates; ignoring deflates them.
 * Alternating may reduce net bias.
 * <p>
 * Same encoding, tier geometry, and history as CDLL4. Only hashAndStore differs.
 * Reuses CDLL4's SBS table (same 1-bit compressed-tier history model).
 *
 * @author Brian Bushnell, Nahida
 * @date April 2026
 */
public final class HalfCappedDynamicLogLog4 extends CardinalityTracker {

	/*--------------------------------------------------------------*/
	/*----------------        Initialization        ----------------*/
	/*--------------------------------------------------------------*/

	HalfCappedDynamicLogLog4(){
		this(2048, 31, -1, 0);
	}

	HalfCappedDynamicLogLog4(parse.Parser p){
		super(p);
		maxArray=new int[(buckets+7)>>>3];
		minZeroCount=buckets;
	}

	HalfCappedDynamicLogLog4(int buckets_, int k_, long seed, float minProb_){
	...
	@Override
	public HalfCappedDynamicLogLog4 copy(){return new HalfCappedDynamicLogLog4(buckets, k, -1, minProb);}
	...
	public final void add(CardinalityTracker log){
		...
		add((HalfCappedDynamicLogLog4)log);
	}

	public void add(HalfCappedDynamicLogLog4 log){
		added+=log.added;
		lastCardinality=-1;
	...
	public final void hashAndStore(final long number){
		...
		if(relTier>=0){
			final int rawTierPart=relTier+1;

			// Overflow handling: CAP_DENOM controls fraction of buckets that cap.
			// CAP_DENOM=2: every other bucket caps (50/50). CAP_DENOM=0: all ignore. CAP_DENOM=1: all cap (CDLL4).
			if(rawTierPart>7){
				if(CAP_DENOM<1 || bucket%CAP_DENOM!=0){
					return; // ignore overflow
				}
				// This bucket caps: fall through with capped value
			}

			final int newTierPart=Math.min(rawTierPart, 7);
			...
			if(newTierPart==oldTierPart-1){
				if(oldHist==0){lastCardinality=-1; writeNibble(bucket, oldNibble|1);}
			}
		}else{
			if(oldTierPart<=1){
				if(oldHist==0){lastCardinality=-1; writeNibble(bucket, oldNibble|1);}
	...
	/** Fraction of buckets that cap overflow. 2=half cap (default), 1=all cap (CDLL4), 0=all ignore, 4=25% cap. */
	public static int CAP_DENOM=2;

	public static final String CF_FILE="?cardinalityCorrectionHCDLL4.tsv.gz";
