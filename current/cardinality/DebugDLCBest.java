...
public class DebugDLCBest{
	...
	public static void main(String[] args){
		...
		final DynamicLogLog2 dll=new DynamicLogLog2(2048, 31, 42, 0);
		final FastRandomXoshiro rng=new FastRandomXoshiro(42);
		...
		final int B=2048;
		final int V=empties;
		final double target=B*0.25; // =512
		System.out.println("DLCBest target V_k = "+target);
		System.out.println();

		final int startTier=lowestActive;
		int vk=V;
		...
		final int bestVk=vks[bestTier];
		final double rawEst=(1L<<bestTier)*(double)B*Math.log((double)B/Math.max(bestVk, 0.5));
		System.out.println();
		...
		final int nativeRelTiers=3;
		final int topStoredTier=startTier+nativeRelTiers-1;
		System.out.println();
		...
		final boolean wouldCorrect=(bestTier>=topStoredTier);
		System.out.println("Would correction fire? "+wouldCorrect);

		if(wouldCorrect){
			final double x=Math.log((double)B/bestVk)/Math.log(2);
			final double err=CorrectionFactor.dlcTierSignedErr(x, CorrectionFactor.DLC_TIER_ERR_DLL2);
			final double cf=1.0/(1.0+err);
			final double corrected=rawEst*cf;
			final int occ=B-bestVk;
		...
		final CardStats s=new CardStats(null, nlz, 0, 0, 0, 0,
