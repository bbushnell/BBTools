...
public class LCHistTrainerBDLL5 {
	...
	static final class TrainerThread extends Thread {
		...
		TrainerThread(long seed, int numTrials, int modBuckets, int words){
			this.seed=seed;
			...
			for(int t=0; t<numTrials; t++){
				...
				while(filledCount<B){
					...
					if(newStored<=0){
						...
						if(oldStored>0){
							int delta=oldStored-newStored;
							if(delta==1||delta==2){
								hist[bucket]=oldHist|(1<<(2-delta));
							}
						}
						continue;
					}
		...
		boolean canPromote(int[] stored, int wordIdx){
		...
		void promoteBank(int[] stored, int[] bank, int wordIdx){
			final int base=wordIdx*6;
			for(int b=0; b<6; b++){stored[base+b]--;}
			bank[wordIdx]++;
		}

		int countAndDecrement(int[] stored, int[] hist, int[] bank, int B, int W){
			int count=0;
			for(int w=0; w<W; w++){
				if(bank[w]>0){
					bank[w]--;
