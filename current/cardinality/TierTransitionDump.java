...
/** Dumps average tier occupancy at each DLL3 tier transition, with DLL4 comparison. */
public class TierTransitionDump {
	public static void main(String[] args) {
		...
		for(int targetMZ=1; targetMZ<=6; targetMZ++){
			double[] r3s=new double[20], c3s=new double[20], r4s=new double[20];
			double[] ovfSum=new double[20]; // storedOverflow accumulator
			int cnt=0;
			for(int di=0; di<N; di++){
				FastRandomXoshiro rng=new FastRandomXoshiro(42+di);
				long s3=rng.nextLong()&Long.MAX_VALUE;
				FastRandomXoshiro rng4=new FastRandomXoshiro(42+di);
				long s4=rng4.nextLong()&Long.MAX_VALUE;
				DynamicLogLog3 d3=new DynamicLogLog3(B,31,s3,0);
			...
			double[] cumRaw3=new double[20], cumRaw4=new double[20];
			...
			for(int t=0;t<20;t++){
				double r=r3s[t]/cnt, c=c3s[t]/cnt, d=r4s[t]/cnt, o=ovfSum[t]/cnt;
