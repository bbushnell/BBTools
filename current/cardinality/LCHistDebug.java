...
/**
 * Debug tool: 1 estimator, 8 buckets, prints state after every add.
 * Exercises ProtoLogLog16c in 2-bit history mode with the LC correction table,
 * printing the per-bucket NLZ/history state and SBS estimate after each insertion.
 *
 * @author Eru, Chloe
 */
public class LCHistDebug{
	...
	public static void main(String[] args){
		...
		final int buckets=8;
		final ProtoLogLog16c pll=new ProtoLogLog16c(buckets, 31, -1, 0);

		// Print table row for filled=1..8 so we can cross-check
		System.out.println("=== LC History Table (rows 1-8) ===");
		final String[] stateNames={"0.00","1.00","1.10","2.00","2.01","2.10","2.11",
			"3+.00","3+.01","3+.10","3+.11"};
		...
		for(int i=0; i<30; i++){
			...
			final short[] maxArray=pll.maxArray;
			int filled=0;
			final StringBuilder states=new StringBuilder();
			for(int b=0; b<buckets; b++){
				final int stored=maxArray[b]&0xFFFF;
				if(stored>0){
					filled++;
					final int nlzShift=16-6; // 6 NLZ bits for PLL16c
					final int absNlz=stored>>>nlzShift;
					final int extra=stored&((1<<nlzShift)-1);
					final int hshift=nlzShift-2; // history shift (2 bits at top of extra)
					final int hist=(extra>>>hshift)&3;
					final int rawNlz=absNlz-1;
					final int nlzBin=Math.min(rawNlz, 3);
					final int si=CorrectionFactor.sbsStateIndex(nlzBin, hist);
			...
			// Compute LC
			final int V=buckets-filled;
			final double lc=V>0 ? buckets*Math.log((double)buckets/V) : Double.POSITIVE_INFINITY;

			// Compute SBS manually -- direct lookup, B matches table
			double sbs=0;
			if(table!=null && filled>0){
				final float[] row=table[Math.min(filled, table.length-1)];

				for(int b=0; b<buckets; b++){
					final int stored=maxArray[b]&0xFFFF;
					if(stored>0){
						final int nlzShift=16-6;
						final int absNlz=stored>>>nlzShift;
						final int extra=stored&((1<<nlzShift)-1);
						final int hshift=nlzShift-2;
						final int hist=(extra>>>hshift)&3;
						final int rawNlz=absNlz-1;
						final int nlzBin=Math.min(rawNlz, 3);
						final int si=CorrectionFactor.sbsStateIndex(nlzBin, hist);
