...
public final class ErtlULL extends CardinalityTracker {
	...
	@Override
	public ErtlULL copy(){return new ErtlULL(buckets, k, -1, minProb);}
	...
	static byte pack(long hashPrefix){
		final int nlz=Long.numberOfLeadingZeros(hashPrefix)+1;
	...
	private double fgraEstimate(){
		...
		if(c0>0 || c4>0 || c8>0 || c10>0){
			final double z=smallRangeEstimate(c0, c4, c8, c10, m);
		...
		if(c4w0>0 || c4w1>0 || c4w2>0 || c4w3>0){
			final double z=largeRangeEstimate(c4w0, c4w1, c4w2, c4w3, m);
			final double rootZ=Math.sqrt(z);
	...
	/**
	 * Merge another ErtlULL via unpack-OR-repack of hash prefix bitsets.
	...
	public final void add(CardinalityTracker log){
		...
		for(int i=0; i<registers.length; i++){
			final long hp=unpack(registers[i])|unpack(other.registers[i]);
			registers[i]=pack(hp);
		}
	}
	...
	/** No-op; ErtlULL uses its own FGRA estimator, not CF tables. */
	...
	private static double sigma(double z){
		...
		while(true){
			final double oldS=s;
			final double nextNextPowZ=nextPowZ*nextPowZ;
	...
	private static double phi(double z, double zSquare){
		...
		while(true){
			...
			final double nextPs=psiPrime(powZ, previousPowZ);
	...
	private static double smallRangeEstimate(long c0, long c4, long c8, long c10, long m){
		final long alpha=m+3*(c0+c4+c8+c10);
		final long beta=m-c0-c4;
		final long gamma=4*c0+2*c4+3*c8+c10;
		final double quadRootZ=(Math.sqrt((double)(beta*beta+4*alpha*gamma))-beta)/(2*alpha);
		final double rootZ=quadRootZ*quadRootZ;
		return rootZ*rootZ;
	}

	private static double largeRangeEstimate(long c4w0, long c4w1, long c4w2, long c4w3, long m){
		final long alpha=m+3*(c4w0+c4w1+c4w2+c4w3);
		final long beta=c4w0+c4w1+2*(c4w2+c4w3);
		final long gamma=m+2*c4w0+c4w2-c4w3;
