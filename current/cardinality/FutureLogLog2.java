 ...
 *   Global advance: when all words have localExp &ge; 1, increment globalExp,
...
public final class FutureLogLog2 extends CardinalityTracker {
	...
	private static double tierCardinality(int absNLZ){
		...
		final double base=tierAvg[tierAvg.length-1];
		final int extra=absNLZ-(tierAvg.length-1);
	...
	private double rawCFEstimate(){
		...
		if(cardCFKeys!=null && cardCFKeys.length>0){
			final double keyScale=(cardCFBuckets>0 && modBuckets!=cardCFBuckets)
				? (double)cardCFBuckets/modBuckets : 1.0;
			final double cf=CorrectionFactor.getCF(sum, sum,
	 ...
	 * Midpoint (50/50) at sqrt(8x80) ~ 25.3.
	 *
	...
	private double blendEstimate(double fllEst, double microEst){
		...
		final double frac=(Math.log(key)-BLEND_LOG_LO)/BLEND_LOG_RANGE;
	...
	/** 16-bit packed banks: [15:12]=localExp, [11:0]=6x2-bit future bitmap. */
	private final short[] words;
	/** Number of 16-bit words (banks). */
	private final int numWords;
	/** Actual bucket count = numWords x 6. May not be a power of 2. */
	...
	public static synchronized void loadCFTable(){
		...
		final FileFormat ff=FileFormat.testInput(path, null, false);
		if(ff==null){setFallbackCF(); return;}
		final ByteFile bf=ByteFile.makeByteFile(ff, 1);
		if(bf==null){setFallbackCF(); return;}

		final java.util.ArrayList<double[]> multRows=new java.util.ArrayList<>();
		final java.util.ArrayList<Double> avgList=new java.util.ArrayList<>();
		...
		for(byte[] line=bf.nextLine(); line!=null; line=bf.nextLine()){
			...
			if(parts[0].equals("tierAvg")){
				final int tier=Integer.parseInt(parts[1]);
				final double avg=Double.parseDouble(parts[2]);
				...
				final double[] row=new double[NUM_EQUIV];
	...
	public static synchronized void loadCardCFTable(){
		...
		final structures.FloatList keys=new structures.FloatList();
		final structures.FloatList vals=new structures.FloatList();
		...
		for(byte[] line=bf.nextLine(); line!=null; line=bf.nextLine()){
			...
			final float key=Float.parseFloat(parts[0]);
			final float cf=Float.parseFloat(parts[1]); // Mean_cf column
			keys.add(key);
			vals.add(cf);
		}
		bf.close();
