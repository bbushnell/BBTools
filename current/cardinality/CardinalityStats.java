...
public class CardinalityStats {
	...
	public double[] toArray(double hybridEst){
		if(count==0){return new double[10];}
		return new double[]{
			meanEstCF,
			hmeanPure  *cf(CorrectionFactor.HMEAN),
			hmeanPureM *cf(CorrectionFactor.HMEANM),
			gmeanEst   *cf(CorrectionFactor.GMEAN),
			hmeanEst   *cf(CorrectionFactor.HLL),
			lcPure,
			hybridEst,
			mwaEst     *cf(CorrectionFactor.MWA),
			medianCorr *cf(CorrectionFactor.MEDCORR),
			mean99Est  *cf(CorrectionFactor.MEAN99)
		};
	}
