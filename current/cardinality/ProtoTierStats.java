 ...
 * <p>
 * Can combine modes: {@code mode=mantissa+history}
 * <p>
 * Usage: {@code java cardinality.ProtoTierStats [inner=32768] [outer=131072] [maxtier=11]
...
public class ProtoTierStats {
	...
	public static void main(String[] args){
		...
		for(int trial=0; trial<outer; trial++){
			...
			final ProtoLogLog16c proto=new ProtoLogLog16c(4, 2, -1, 0); // 4 buckets (min valid)

			for(int card=1; card<=inner; card++){
				final long val=Tools.hash64shift(seed+card);
				proto.add(val);

				final int stored=proto.maxArray[0]&0xFFFF;
				if(stored>0){
					final int absNlz=(stored>>>nlzShift)-1;
		...
		final StringBuilder header=new StringBuilder("Tier\tTotal");
		...
		for(int t=0; t<=maxTier; t++){
			...
			final double tierAvg=tierSum/tierTotal;

			final StringBuilder sb=new StringBuilder();
			sb.append(t).append('\t').append(tierTotal);
			for(int s=0; s<numStates; s++){
				final double freq=count[t][s]/(double)tierTotal;
				sb.append('\t').append(String.format("%.4f", freq));
				if(count[t][s]>10){
					final double stateAvg=sum[t][s]/count[t][s];
					final double cf=Math.log(stateAvg/tierAvg)/Math.log(2);
		...
		long grandTotal=0;
		final double[] grandCF=new double[numStates];
		...
		final StringBuilder cfLine=new StringBuilder("# CF=[");
		double minCF=Double.MAX_VALUE, maxCF=-Double.MAX_VALUE;
		for(int s=0; s<numStates; s++){
			final double cf=grandCF[s]/(grandTotal>0 ? grandTotal : 1);
