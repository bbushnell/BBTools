...
/**
 * LC history CF table trainer for EDLL8 (expanded tier geometry, 3-bit history).
 * Tier = (2*rawNlz + mantissa) / 4, with mantissa = key_fraction >= (2-sqrt(2)).
 * 3-bit history: carry-shift with HIST_CARRY=8, HIST_MASK=7.
 *
 * Forked from LCHistTrainerCDLL4 (Chloe). Changes:
 *   - tierOf() uses halfNlz/4 (expanded, TIER_SCALE=2.0)
 *   - Default hbits=3
 *   - Modular bucket addressing (Long.remainderUnsigned) instead of power-of-2
 *
 * @author Nowi
 * @date April 2026
 */
public class LCHistTrainerEDLL8 {
	...
	/** Compute EDLL8 absolute tier from a hashed key. Expanded: tier = halfNlz. */
				...
				while(filledCount<B){
					...
					if(!filled[bucket]){
						...
						if(delta>0){
							...
							hist[bucket]=((oldH|hcarry)>>delta)&hmask;
							tierArr[bucket]=tier;
						}else if(delta<0){
	...
	public static void main(String[] args){
		...
		System.err.println("LCHistTrainerEDLL8: buckets="+buckets+" hbits="+hbits+
		...
		try{
			...
			pw.println("#LC_HISTORY_TABLE_EDLL8");
			pw.println("#Buckets\t"+buckets);
			pw.println("#HistBits\t"+hbits);
			pw.println("#TierGeom\tEDLL8_expanded_halfNlz");
