...
/**
 * LC history CF table trainer for VCDLL4 (half-NLZ tier geometry, variable history).
 * Tiers 0-5: 1-bit history (same as CDLL4). Tiers 6-9: no history (always 0).
 ...
 *
 * Usage: java cardinality.LCHistTrainerVCDLL4 buckets=2048 iters=1m out=table.tsv.gz
 *
 * @author Noire
 * @date April 2026
 */
public class LCHistTrainerVCDLL4 {
	...
	/** Maximum tier part before extended encoding (no history). */
	static final int EXTENDED_TIER_THRESHOLD=6;
	...
	static final class TrainerThread extends Thread {
		...
		void runInner(){
			...
			for(int t=0; t<numTrials; t++){
				...
				while(filledCount<B){
					...
					if(!filled[bucket]){
						tierArr[bucket]=tier;
						// First fill: tiers >= EXTENDED_TIER_THRESHOLD have no history
						hist[bucket]=(tier>=EXTENDED_TIER_THRESHOLD) ? 0 : 0;
						filled[bucket]=true;
						filledCount++;
						...
						if(delta>0){
							tierArr[bucket]=tier;
							if(tier>=EXTENDED_TIER_THRESHOLD){
								// Extended tier: no history
								hist[bucket]=0;
							}else{
								// Standard tier: carry-shift history
								final int oldH=hist[bucket]&hmask;
								hist[bucket]=((oldH|(1<<hbits))>>delta)&hmask;
							}
						}else if(delta<0){
							// delta == -1: set history if current tier is standard
							if(oldTier<EXTENDED_TIER_THRESHOLD){
								final int diff=-delta;
								if(diff>=1 && diff<=hbits){
									hist[bucket]|=1<<(hbits-diff);
								}
							}
							// Extended tier: no-op (no history to set)
						}
					}
				}
			}
		...
		void snapshot(int filledCount, int[] tier, int[] hist, int[] distinct, boolean[] filled){
			for(int b=0; b<buckets; b++){
				if(filled[b]){
					// For extended tiers, always report hist=0
					final int effectiveHist=(tier[b]>=EXTENDED_TIER_THRESHOLD) ? 0 : hist[b];
					int si=stateIndex(tier[b], effectiveHist, hbits);
	...
	public static void main(String[] args){
		...
		System.err.println("LCHistTrainerVCDLL4: buckets="+buckets+" hbits="+hbits+
		...
		try{
			...
			pw.println("#LC_HISTORY_TABLE_VCDLL4");
			pw.println("#Buckets\t"+buckets);
			pw.println("#HistBits\t"+hbits);
			pw.println("#TierGeom\tVCDLL4_half_nlz_variable_hist");
