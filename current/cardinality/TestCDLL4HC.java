...
public class TestCDLL4HC {
    ...
    public static void main(String[] args){
        ...
        CompressedDynamicLogLog4 c=new CompressedDynamicLogLog4(B, 31, 12345L, 0);
        ...
        int[] nlzBucketCount=new int[64];
        int[][] nlzHbitSet=new int[1][64]; // 1-bit history
        int[] histAtTier=new int[64]; // count of hist=1 per tier
        int[] totalAtTier=new int[64];

        for(int i=0; i<B; i++){
            int nib=c.readNibble(i);
            int tp=nib>>>1;
            int hist=nib&1;
            int absTier;
            if(tp>0){
        ...
        for(int t=0; t<=maxTier+1; t++){
            ...
            int beff=nlzBucketCount[sourceTier];
            int unseen=beff-nlzHbitSet[0][sourceTier];
            if(beff<2 || unseen<1 || unseen>=beff) continue;

            double est=Math.pow(2.0, (t+1)*TIER_SCALE)*B*Math.log((double)beff/unseen);
            double ratio=est/trueCard;
