...
public class SendClade extends CladeObject {
	...
	public SendClade(String[] args){
		...
		for(int i=0; i<args.length; i++){
			...
			}else if(a.equals("mtload")){
				mtload=Parse.parseBoolean(b);
			}else if(a.equals("sfload")){
		...
		if(mtload) {
			...
			for(String fname : in){
				...
				if(sfload) {
					...
					ArrayList<Clade> clades=loaderSF.loadFile(fname, perContig, minlen, maxReads, true);
					sequencesProcessed+=loaderSF.readsProcessed;
	...
	private long processFastaFile(FileFormat ff, TextStreamWriter tsw){
		...
		//Load sequences using standard BBTools Streamer
		final Streamer cris;
		{
			cris=StreamerFactory.getReadInputStream(maxReads, false, ff, null, -1);
			cris.start();
		}
		assert(cris != null) : "Streamer creation failed for " + ff.name();
