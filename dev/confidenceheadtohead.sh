...
Description: Head-to-head: does one continuous depth-regression net calibrate as well as the 8-net boolean confidence bundle, same data and metric?

Usage: confidenceheadtohead.sh <args>   (arguments are parsed by clade.ConfidenceHeadToHead; run the class or read
...
launch(){
  local CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP clade.ConfidenceHeadToHead $@"
  echo "$CMD" >&2
  java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" clade.ConfidenceHeadToHead "$@"
}
launch "$@"
