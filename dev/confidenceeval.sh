...
Description: Evaluates QuickClade per-hit confidence calibration CONDITIONALLY on query length and similarity-to-reference (KID).

Usage: confidenceeval.sh <args>   (arguments are parsed by clade.ConfidenceEval; run the class or read
...
launch(){
  local CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP clade.ConfidenceEval $@"
  echo "$CMD" >&2
  java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" clade.ConfidenceEval "$@"
}
launch "$@"
