...
Description: Converts a single-continuous-label confidence vector file into multi-output (per-level) label format.

Usage: confidencemultilabel.sh <args>   (arguments are parsed by clade.ConfidenceMultiLabel; run the class or read
...
launch(){
  local CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP clade.ConfidenceMultiLabel $@"
  echo "$CMD" >&2
  java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" clade.ConfidenceMultiLabel "$@"
}
launch "$@"
