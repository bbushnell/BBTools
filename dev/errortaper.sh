...
Description: Subsamples a training vector file by prediction error (keep all hard examples, taper the easy ones).

Usage: errortaper.sh <args>   (arguments are parsed by ml.ErrorTaper; run the class or read
...
launch(){
  local CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP ml.ErrorTaper $@"
  echo "$CMD" >&2
  java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" ml.ErrorTaper "$@"
}
launch "$@"
