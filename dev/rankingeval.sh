...
Description: Evaluates ranking nets against compositeScore on benchmark data.

Usage: rankingeval.sh <args>   (arguments are parsed by ml.RankingEval; run the class or read
...
launch(){
  local CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP ml.RankingEval $@"
  echo "$CMD" >&2
  java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" ml.RankingEval "$@"
}
launch "$@"
