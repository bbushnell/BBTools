...
Description: Packages a single multi-output confidence net (48 inputs -> 8 per-level heads) into a .bbnets bundle CladeConfidence loads as a one-net replacement.

Usage: confidencemultibundle.sh <args>   (arguments are parsed by clade.ConfidenceMultiBundle; run the class or read
...
launch(){
  local CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP clade.ConfidenceMultiBundle $@"
  echo "$CMD" >&2
  java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" clade.ConfidenceMultiBundle "$@"
}
launch "$@"
