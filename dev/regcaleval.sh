...
Description: Ranks continuous RegressionTrainer nets by CALIBRATION rather than MSE over a directory of net_*.bbnet.

Usage: regcaleval.sh <args>   (arguments are parsed by ml.RegCalEval; run the class or read
...
launch(){
  local CMD="java $EA $EOOM $SIMD $XMX $XMS -cp $CP ml.RegCalEval $@"
  echo "$CMD" >&2
  java $EA $EOOM $SIMD $XMX $XMS -cp "$CP" ml.RegCalEval "$@"
}
launch "$@"
